...
class VehiclesModule extends Module {
  ...
  List<Bind<Object>> binds = [
    ...
    Bind.factory<VehicleListController>(
      (i) => VehicleListController(usecase: i.get<VehiclesUsecase>()),
    ),
    Bind.lazySingleton<AddVehicleController>(
      (i) => AddVehicleController(usecase: i.get<VehiclesUsecase>()),
    ),

    // usecases
    Bind.lazySingleton<VehiclesUsecase>(
      (i) => VehiclesUsecase(repository: i.get<VehicleRepository>()),
    ),

    // repository
    Bind.lazySingleton<VehicleRepository>(
      (i) => VehicleRepository(datasource: i.get<VehiclesDatasource>()),
    ),
