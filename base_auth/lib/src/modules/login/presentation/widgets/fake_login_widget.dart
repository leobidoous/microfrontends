...
import '../stores/fake_login_store.dart';

class FakeLogin extends StatefulWidget {
  final BasePath loginFoward;
  const FakeLogin({Key? key, required this.loginFoward}) : super(key: key);
...
class _FakeLoginState extends State<FakeLogin> {
  final store = Modular.get<FakeLoginStore>();

  void fakeLogin() async {
    store.login().then((value) async {
      if (store.state) {
